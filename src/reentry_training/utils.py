        ...
        for line in batch_lst:
            ...
            for c in range(self.C):
                ...
                if self.pretrained_v:
                    ...
                    visual_path=visual_path.replace('visual_embedding/sync/sync_av', 'orig')
                    length = math.floor(min_length/self.sampling_rate*25)
...
def get_dataloader(args, partition):
    datasets = dataset(
                ...
                partition=partition,
                mix_no=args.C,
                pretrained_v=args.pretrained_v)
...
class Loss_amSoftmax(nn.Module):
    def __init__(self, nOut=256, nClasses=800, margin=0.3, scale=30):
        super(Loss_amSoftmax, self).__init__()
        self.m = margin
        self.s = scale
        self.ce = nn.CrossEntropyLoss()
        print('Initialised amSoftmax Loss')

    def forward(self, costh, label=None):
